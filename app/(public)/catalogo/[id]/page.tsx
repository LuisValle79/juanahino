    ...
    const loadVehicle = async () => {
      try {
        ...
        if (data.success && data.vehicle) {
          ...
          const relatedResponse = await fetch(`/api/vehicles?type=${data.vehicle.tipo}`)
          const relatedData = await relatedResponse.json()
          
          if (relatedData.success) {
            const filteredRelated = relatedData.vehicles
              .filter((v: Vehicle) => v.id !== data.vehicle.id)
              .slice(0, 3)
      ...
      <div className="bg-muted/30 py-4">
        <div className="container mx-auto px-4">
          <nav className="text-sm">
            ...
            <Link href="/catalogo" className="text-muted-foreground hover:text-primary">Catálogo</Link>
            <span className="mx-2 text-muted-foreground">/</span>
                ...
                <CardContent className="space-y-4">
                  {relatedVehicles.map((relatedVehicle) => (
                    <Link 
                      ...
                      href={`/catalogo/${relatedVehicle.id}`}
                      className="block"
                    >
